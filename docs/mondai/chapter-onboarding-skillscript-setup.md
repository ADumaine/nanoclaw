...
- **The Gmail stub credential files must be well-formed, parseable JSON matching `gmail-mcp`'s expected shape — not the literal string `"onecli-managed"`.** Confirmed on production, 2026-08-15: `gmail-mcp` (`@gongrzhe/server-gmail-autoauth-mcp`) calls `JSON.parse()` on both files at startup. A bare non-JSON string throws a `SyntaxError` that crashes the process, which surfaces through the `RemoteMcpConnector` as `RemoteMcpDispatchError: child process exited mid-request (code=1)` on the *next* tool call, not at connector setup — confusing, since the crash and the error report happen on different requests. Use instead:
  - `gcp-oauth.keys.json`: `{"installed":{"client_id":"onecli-managed","client_secret":"onecli-managed","redirect_uris":["http://localhost:3000/oauth2callback"]}}`
  - `credentials.json`: **not** `{}` — see next point, it needs to look populated too.
- **`credentials.json` also can't be an empty object `{}`.** `gmail-mcp` uses the `googleapis` Node client (`google.auth.OAuth2`), which refuses to issue *any* HTTP request locally if no access/refresh token is set on the client object — `Error: No access, refresh token, API key or refresh handler callback is set.` This check happens client-side, before any network call is attempted, so `onecli`'s transparent MITM injection (which only intercepts requests that actually go out over the wire) never gets a chance to help — the request never leaves the process. The fix is a well-formed but fake-valued token object, since the real Authorization header gets swapped in transparently at the proxy layer regardless of what's set locally:
  ```json
  {"access_token":"onecli-managed","refresh_token":"onecli-managed","scope":"https://www.googleapis.com/auth/gmail.modify","token_type":"Bearer","expiry_date":99999999999999}
  ```
  **Requires a `docker compose restart dashboard` to take effect** — the connector's `gmail-mcp` subprocess is spawned once and its in-memory `OAuth2Client` doesn't re-read the credentials file on later calls; editing the file alone doesn't fix an already-running connector.
  **Dev almost certainly has this exact same latent bug and needs to check independently — not by copying anything.** These stub files are per-host local placeholders, never git-tracked and never copied between hosts (unlike the skills themselves). Dev's stub files were set up following this doc's *old* guidance (bare `"onecli-managed"` string) before tonight's fix existed, so dev's own Gmail-via-skillscript connector is very likely silently broken the same way right now — a `$ gmail.send_email` call there would either crash the connector (bad `gcp-oauth.keys.json`) or fail the local `googleapis` client precondition (empty `credentials.json`), and either way any skill trusting the result unconditionally (see next point) would still report "sent". This has probably never been caught on dev because no one forced a real send through it and cross-checked the OneCLI gateway's own request log the way this incident did on production. **Action for dev:** apply the same two JSON-shaped stub files above to dev's own `/data/gmail-mcp/*.json`, `docker compose restart dashboard` on dev, then verify with a real test send (same recipe as "Verification (real send...)" below) — checking the gateway log for an actual `POST .../messages/send`, not just the skill's own reported text.
- **Known bug, not yet fixed as of 2026-08-15: every skillscript skill that calls `$ gmail.send_email` trusts the result unconditionally, with no check for an error string.** `gmail-mcp` tool errors (e.g. the credentials error above) come back as ordinary MCP tool-call *content* — a normal string in the `-> VAR` binding — not a protocol-level failure. So the skill's `$` op completes without raising an `OpError`, and whatever code runs next has no way to tell a real send apart from a swallowed error unless it explicitly checks the string. This produced a real false positive in production (Nassau chapter, pipeline `663da938-...`, 2026-08-15): `onboarding-dashboard-trigger` reported "Welcome email sent" while no email had gone out at all, and it was only caught by cross-checking the OneCLI gateway's own request log for a real `POST .../messages/send`.

  **What dev needs to do:**
  1. **Confirm the exact error-string shape(s) `gmail-mcp`'s `send_email` tool can return**, beyond the one reproduced here (`"Error: No access, refresh token, API key or refresh handler callback is set."`) — e.g. what a real Gmail API failure (invalid recipient, quota, revoked auth) looks like in the tool's returned text, so the check isn't tuned to just this one string. Check `@gongrzhe/server-gmail-autoauth-mcp`'s source (`dist/index.js`) for how it formats send errors, or force a couple of real failure modes against a throwaway skill and observe.
  2. **Fix all four affected call sites** (confirmed via `grep -l 'gmail.send_email' data/skills/*.skill.md` on production, 2026-08-15 — dev's skill store may have more or different filenames, re-run the same grep there):
     - `onboarding-dashboard-trigger.skill.md` — `branch` target, `-> SEND`, followed by `$set OUTCOME = "sent"`.
     - `onboarding-go-live.skill.md` — `render_and_send_golive` target, `-> SEND_RESULT`, followed by `$set GOLIVE_EMAIL_SENT = "${SEND_RESULT}"` (this one doesn't even set a boolean/status — it stores the raw string, error or not, directly into the field a downstream reader would treat as a success flag).
     - `onboarding-daily-sweep.skill.md` — two call sites, `-> SEND` (follow-up email) and `-> SEND2` (nag email), same unconditional-success pattern.
  3. **In each case, check the bound variable for the error marker from step 1 before treating the send as successful** — consult skillscript's own op/string-matching reference for the exact conditional syntax available (this codebase's existing skills only use `==`/`!=` equality checks against known literal values, e.g. `if ${TELEGRAM_ID} != ""` in `onboarding-go-live.skill.md`'s `notify_cal_mgr` target — there's no existing example here of a substring/prefix check, so confirm what skillscript actually supports rather than assuming). On a detected error, the skill should report a real failure (non-"sent" `OUTCOME`, an explicit error in `REPLY_TEXT`) instead of claiming success.
  4. **Test on dev by forcing the same failure** (e.g. temporarily point `GMAIL_CREDENTIALS_PATH` at an empty `{}` file again, or another known-bad state) and confirm the fixed skill now reports the failure honestly instead of "sent".
  5. **Deploy to production the same manual way as the rest of `data/skills/`** — these skills aren't git-tracked (see "What lives where" above), so re-copy the fixed `.skill.md` + `.versions.jsonl` files, or re-run the equivalent `skill_write` RPC calls, directly against production's skillscript instance. No NanoClaw-side restart needed — skillscript reads its skill store fresh on every `execute_skill` call.
