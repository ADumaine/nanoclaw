 ...
 *
 * Each app_id maps to one NanoClaw messaging group / agent group pair.
 * Multiple app_ids (e.g. dev, staging, prod) can share one NanoClaw instance.
 *
 * Required .env vars:
 ...
 *                                       Allows multiple API servers (e.g. dev, staging) to
...
registerChannelAdapter(CHANNEL_TYPE, {
  factory: () => {
    ...
    // Allows dev and staging API servers to share one NanoClaw instance.
    ...
    async function handleMessage(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
      ...
      await setupConfig!.onInbound(platformId, thread_id!, {
        ...
        content: {
          ...
          user_context: {
            ...(user_context ?? {}),
            // Prefer explicit auth_id from caller.
